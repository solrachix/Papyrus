...
const SidebarRight: React.FC<SidebarRightProps> = ({ engine }) => {
  const { 
    ...
    uiTheme, setSearch, setDocumentState, triggerScrollToPage, annotations
  } = useViewerStore();
    ...
    <div className={`w-80 border-l flex flex-col h-full shrink-0 transition-colors duration-200 shadow-2xl z-40 ${isDark ? 'bg-[#1a1a1a] border-[#333]' : 'bg-white border-gray-200'}`}>
      <div className={`p-4 border-b flex items-center justify-between shrink-0 ${isDark ? 'border-[#333]' : 'border-gray-100'}`}>
        <div className="flex space-x-6">
          <button onClick={() => toggleSidebarRight('search')} className={`text-[10px] font-black uppercase tracking-widest pb-1 transition-all ${sidebarRightTab === 'search' ? 'text-blue-500 border-b-2 border-blue-500' : 'text-gray-400'}`}>Busca</button>
          <button onClick={() => toggleSidebarRight('annotations')} className={`text-[10px] font-black uppercase tracking-widest pb-1 transition-all ${sidebarRightTab === 'annotations' ? 'text-blue-500 border-b-2 border-blue-500' : 'text-gray-400'}`}>Notas</button>
        </div>
        ...
        {sidebarRightTab === 'search' ? (
          <div className="space-y-4">
            <form onSubmit={handleSearch} className="relative mb-6">
              ...
              />
              <button type="submit" className="absolute right-3 top-2.5 text-gray-400 hover:text-blue-500 transition-colors">
            ...
            {!isSearching && searchResults.map((res, idx) => (
              <div 
                ...
                className={`p-4 rounded-xl border-2 cursor-pointer transition-all group hover:scale-[1.02] ${idx === activeSearchIndex ? 'border-blue-500 bg-blue-500/10 shadow-lg' : isDark ? 'border-[#333] hover:border-[#555] bg-[#222]' : 'border-gray-50 hover:border-gray-200 bg-gray-50/50 hover:bg-white'}`}
              >
                <div className="flex items-center justify-between mb-2">
                  <span className={`text-[10px] font-black uppercase tracking-tighter ${idx === activeSearchIndex ? 'text-blue-500' : 'text-gray-400'}`}>PÁGINA {res.pageIndex + 1}</span>
                  <svg className={`w-3 h-3 transition-transform ${idx === activeSearchIndex ? 'text-blue-500' : 'text-gray-300'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M9 5l7 7-7 7" /></svg>
                </div>
                ...
                <div key={ann.id} className={`p-4 rounded-xl border group transition-all cursor-pointer ${isDark ? 'bg-[#222] border-[#333] hover:border-[#444]' : 'bg-white border-gray-100 shadow-sm hover:shadow-md'}`}>
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-[10px] font-black text-blue-500">P{ann.pageIndex + 1}</span>
                    </div>
