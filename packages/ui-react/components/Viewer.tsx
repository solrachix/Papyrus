...
const Viewer: React.FC<ViewerProps> = ({ engine }) => {
  const { pageCount, currentPage, zoom, activeTool, uiTheme, scrollToPageSignal, setDocumentState, accentColor, annotationColor, setAnnotationColor } = useViewerStore();
  ...
  const handlePageMeasured = (pageIndex: number, size: { width: number; height: number }) => {
    setPageSizes((prev) => {
    ...
    >
      <div className="flex flex-col items-center gap-6 w-full min-w-0">
        {pages.map(idx => (
          <div
            key={idx}
            ref={(element) => { pageRefs.current[idx] = element; }}
            data-page-index={idx}
      ...
      <div className={`papyrus-tool-dock sticky ${toolDockPosition} w-full flex justify-center pointer-events-none z-[70]`}>
        <div className={`pointer-events-auto shadow-2xl rounded-2xl p-2 flex items-center border z-[80] ${isDark ? 'bg-[#2a2a2a]/90 border-[#3a3a3a] backdrop-blur-xl' : 'bg-white/95 border-gray-100 backdrop-blur-md'}`}>
          {tools.map(tool => (
            <button
              key={tool.id}
              title={tool.name}
          ...
          >
            <span className="w-5 h-5 rounded-full" style={{ backgroundColor: annotationColor }} />
          </button>
          {colorPickerOpen && (
            <div className={`absolute bottom-full left-1/2 -translate-x-1/2 mb-3 w-48 rounded-xl border p-3 shadow-2xl overflow-hidden ${isDark ? 'bg-[#1f1f1f] border-[#333]' : 'bg-white border-gray-200'}`}>
              <div className="grid grid-cols-4 gap-2 mb-3">
                {colorPalette.map((color) => (
                  <button
                    key={color}
                    onClick={() => { setAnnotationColor(color); setColorPickerOpen(false); }}
              ...
              <div className="flex items-center gap-2 w-full">
                <span className="text-[10px] uppercase tracking-widest text-gray-400 shrink-0">Hex</span>
                <input
                  type="text"
