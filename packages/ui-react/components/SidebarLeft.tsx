...
const SidebarLeft: React.FC<SidebarLeftProps> = ({ engine }) => {
  const { 
    ...
    outlineSearchQuery, setOutlineSearch
  } = useViewerStore();
    ...
    <div className={`w-72 border-r flex flex-col h-full shrink-0 overflow-hidden transition-colors duration-200 ${isDark ? 'bg-[#2a2a2a] border-[#3a3a3a]' : 'bg-[#fcfcfc] border-gray-200'}`}>
      <div className={`p-4 border-b flex flex-col space-y-4 ${isDark ? 'border-[#3a3a3a]' : 'border-gray-100'}`}>
        ...
        <div className="flex gap-1">
          <button onClick={() => setSidebarLeftTab('thumbnails')} className={`p-2 rounded-md ${sidebarLeftTab === 'thumbnails' ? (isDark ? 'bg-white/10 text-white' : 'bg-white text-blue-600 shadow-sm border border-gray-200') : 'text-gray-400'}`}>
          ...
          <button onClick={() => setSidebarLeftTab('summary')} className={`p-2 rounded-md ${sidebarLeftTab === 'summary' ? (isDark ? 'bg-white/10 text-white' : 'bg-white text-blue-600 shadow-sm border border-gray-200') : 'text-gray-400'}`}>
        ...
        {sidebarLeftTab === 'thumbnails' ? (
          <div className="space-y-1">
            {Array.from({ length: pageCount }).map((_, idx) => (
              <Thumbnail key={idx} engine={engine} pageIndex={idx} isDark={isDark} active={currentPage === idx + 1} onClick={() => { engine.goToPage(idx + 1); setDocumentState({ currentPage: idx + 1 }); triggerScrollToPage(idx); }} />
            ))}
          </div>
        ) : (
          <div className="flex flex-col space-y-0.5">
            {outline.map((item, i) => (<OutlineNode key={i} item={item} engine={engine} isDark={isDark} />))}
          </div>
        )}
      </div>
    </div>
