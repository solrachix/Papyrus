...
interface PageRendererProps {
  ...
  onMeasuredSize?: (pageIndex: number, size: { width: number; height: number }) => void;
}

const PageRenderer: React.FC<PageRendererProps> = ({ engine, pageIndex, availableWidth, onMeasuredSize }) => {
  ...
  const { 
    ...
    selectedAnnotationId, setSelectedAnnotation, accentColor, annotationColor,
    searchQuery, searchResults, activeSearchIndex
  } = useViewerStore();
  const renderTargetType = engine.getRenderTargetType?.() ?? 'canvas';
  const isElementRender = renderTargetType === 'element';
  const textMarkupTools = new Set(['highlight', 'underline', 'squiggly', 'strikeout']);
  ...
  useEffect(() => {
    ...
    const render = async () => {
      ...
      try {
        ...
        const canvasRenderScale = isElementRender ? 1.0 : RENDER_SCALE * fitScale;
  ...
  useEffect(() => {
    ...
    const existingMarks = Array.from(layer.querySelectorAll('mark.papyrus-search-hit')) as HTMLElement[];
    ...
    if (selectionText && textLayerRef.current && containerRef.current && selection && selection.rangeCount > 0) {
      ...
      if (textLayerRef.current.contains(range.commonAncestorContainer)) {
        ...
        const uniqueRects = rects.filter((rect, index, list) => {
          ...
          return list.findIndex((r) => `${Math.round(r.x * 10000)}-${Math.round(r.y * 10000)}-${Math.round(r.width * 10000)}-${Math.round(r.height * 10000)}` === key) === index;
        });
        const mergedRects = uniqueRects.reduce((acc: typeof uniqueRects, rect) => {
          const target = acc.find((r) => {
            const closeY = Math.abs(r.y - rect.y) < 0.002 && Math.abs(r.height - rect.height) < 0.002;
            const overlaps = rect.x <= r.x + r.width + 0.002 && rect.x + rect.width >= r.x - 0.002;
  ...
  const renderInk = () => {
    ...
    return (
      <svg
        ...
        preserveAspectRatio="none"
      >
        <path d={d} fill="none" stroke={ann.color} strokeWidth={0.008} strokeLinecap="round" strokeLinejoin="round" />
      </svg>
    );
  };
