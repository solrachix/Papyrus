...
const PageRenderer: React.FC<PageRendererProps> = ({ engine, pageIndex }) => {
  ...
  const [elementPageSize, setElementPageSize] = useState<{
    width: number;
    height: number;
  } | null>(null);
  ...
  const isSearchActive =
    sidebarRightOpen && sidebarRightTab === "search" && searchQuery.length > 1;
  ...
  const supportsTextOverlay = renderTargetType === "canvas";
  const renderZoomDependency = isElementTarget ? 1 : zoom;
  const renderRotationDependency = isElementTarget ? 0 : rotation;

  useEffect(() => {
    let active = true;
    const render = async () => {
      const target = isElementTarget ? elementRef.current : canvasRef.current;
      if (!target) return;
      setLoading(true);
      try {
        await engine.renderPage(pageIndex, target, 2.0);
        const text = await engine.getTextContent(pageIndex);
        const dimensions = await engine.getPageDimensions(pageIndex);
  ...
  const handleCanvasClick = (e: React.MouseEvent) => {
    ...
    const rect = (
      isElementTarget ? elementRef.current : canvasRef.current
    ...
    const newAnnotation: Annotation = {
      ...
      color:
        activeTool === "highlight"
          ...
          : "#3b82f6",
      content:
        activeTool === "text" || activeTool === "comment" ? "" : undefined,
      createdAt: Date.now(),
    };
  ...
  const getPageFilter = () => {
    switch (pageTheme) {
      case "sepia":
        return "sepia(0.5) contrast(1.1) brightness(0.95)";
      case "dark":
        return "invert(0.9) hue-rotate(180deg) brightness(1.2) contrast(0.9)";
      case "high-contrast":
  ...
  const baseElementWidth = elementPageSize?.width ?? 640;
  const baseElementHeight = elementPageSize?.height ?? 900;
  const pageContainerStyle = isElementTarget
    ? {
        scrollMarginTop: "20px",
        width: `${Math.max(1, Math.round(baseElementWidth * zoom))}px`,
        height: `${Math.max(1, Math.round(baseElementHeight * zoom))}px`,
      }
    : { scrollMarginTop: "20px" };

  const searchHighlights =
    supportsTextOverlay && isSearchActive
      ? textContent.filter((item) =>
      ...
      {isElementTarget ? (
        <div
          ...
          style={{
            ...
            width: `${baseElementWidth}px`,
            height: `${baseElementHeight}px`,
        ...
        <div className="absolute inset-0 pointer-events-none opacity-50 z-10">
          {searchHighlights.map((item, i) => (
            <div
              ...
              style={{
                left: `${
                  ((item.transform[4] / (canvasRef.current?.width || 600)) *
                    100 *
                    (canvasRef.current?.width || 1)) /
                  3.5
                }%`,
                bottom: `${
                  ((item.transform[5] / (canvasRef.current?.height || 800)) *
                    100 *
                    (canvasRef.current?.height || 1)) /
                  4.5
                }%`,
                width: `${
                  (item.width / (canvasRef.current?.width || 600)) * 100 * 150
                }%`,
                height: `${
                  (item.height / (canvasRef.current?.height || 800)) * 100 * 120
                }%`,
                minHeight: "1.2%",
                minWidth: "2%",
              }}
  ...
  return (
    <div
      className={`absolute pointer-events-auto transition-all duration-200 ${
        isSelected
          ? "ring-2 ring-blue-500 shadow-2xl z-30 scale-105"
      ...
      style={{
        ...
        borderBottom:
          ann.type === "strikeout" ? `3px solid ${ann.color}` : "none",
      }}
      onClick={(e) => {
        e.stopPropagation();
        onSelect();
