...
const Thumbnail: React.FC<{
  ...
  isDark: boolean;
  onClick: () => void;
}> = ({ engine, pageIndex, active, isDark, onClick }) => {
  ...
  useEffect(() => {
    ...
    if (target) {
      engine.renderPage(pageIndex, target, 0.15);
    }
  }, [engine, pageIndex, isElementTarget]);
...
const OutlineNode: React.FC<{
  ...
  return (
    <div className="flex flex-col">
      <div
        className={`flex items-center py-1.5 px-3 rounded-md cursor-pointer transition-colors group ${
...
const SidebarLeft: React.FC<SidebarLeftProps> = ({ engine }) => {
  ...
  return (
    <div
      className={`w-72 border-r flex flex-col h-full shrink-0 overflow-hidden transition-colors duration-200 ${
        isDark
        ...
        {sidebarLeftTab === "summary" && (
          <div className="relative">
            <input
              ...
              className={`w-full py-1.5 px-3 pr-8 rounded text-sm outline-none border transition-colors ${
                isDark
            ...
            {outline.length > 0 ? (
              ...
              <div className="text-center py-20 opacity-30 px-4">
                ...
                <p className="text-[10px] leading-tight">
                  Este documento não possui metadados de índice definidos.
                </p>
              </div>
            )}
          </div>
