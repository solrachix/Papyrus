...
const SUPPORTED_PROFILES = new Set(['text', 'image-heavy', 'varied-sizes', 'text-heavy']);

function escapePdfText(value) {
  return value.replaceAll('\\', '\\\\').replaceAll('(', '\\(').replaceAll(')', '\\)');
}

function createImageData(page, width = 256, height = 256) {
...
export function createBenchmarkPdf({ pageCount = 1000, profile, pageSizeForPage } = {}) {
  if (!Number.isInteger(pageCount) || pageCount < 1) {
    throw new Error('pageCount must be a positive integer');
  }
  if (profile !== undefined && !SUPPORTED_PROFILES.has(profile)) {
    throw new Error(`unsupported profile: ${profile}`);
  }

  // The mobile fixtures use the callback to define their exact page sizes and
  // retain the original text payload used by the PR15 hashes.
  const legacyMobilePayload = profile === undefined && pageSizeForPage !== undefined;
  const activeProfile = profile ?? 'text';
  const isImageHeavy = activeProfile === 'image-heavy';
  ...
  const resolvePageSize = (page) => {
    if (pageSizeForPage) return pageSizeForPage(page);
    if (activeProfile !== 'varied-sizes') return DEFAULT_PAGE_SIZE;
    return [
      [612, 792],
      [400, 1100],
      [900, 600],
  ...
  const baseLines = (page) => legacyMobilePayload
    ? [
    ...
    : [
        `Papyrus benchmark page ${page} of ${pageCount}`,
        `Synthetic ${activeProfile} PDF generated locally for repeatable engine measurements.`,
        'Rust core candidate: parsing, text extraction, search and outline.',
        `Search token: papyrus-benchmark-${page % 17}`,
      ];

  const textLines = (page) => {
    if (activeProfile !== 'text-heavy') return baseLines(page);
    return [
      ...baseLines(page),
      ...Array.from(
        { length: 80 },
  ...
  for (let page = 1; page <= pageCount; page += 1) {
    ...
    const commands = legacyMobilePayload
      ? ['BT', '/F1 16 Tf', '72 740 Td']
      : ['BT', '/F1 16 Tf', `72 ${height - 52} Td`];
    for (const [index, line] of lines.entries()) {
      if (index > 0) commands.push(legacyMobilePayload ? '0 -28 Td' : '0 -18 Td');
      commands.push(`(${escapePdfText(line)}) Tj`);
    }
    commands.push('ET');
    if (isImageHeavy) commands.push('q', '400 0 0 400 106 196 cm', '/Im1 Do', 'Q');
...
export function writeBenchmarkPdf(outputPath, options = {}) {
  ...
  return {
    ...
    pageCount: options.pageCount ?? 1000,
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  console.log(JSON.stringify(writeBenchmarkPdf(outputPath, { pageCount, profile })));
}
